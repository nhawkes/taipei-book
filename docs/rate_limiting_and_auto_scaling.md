...
Not all requests are equally valuable. For a shopping website checkout requests are probably more important than browsing requests. And logged-in users are probably more important than logged-out requests. There's also usually some concept of fairness. If one user or tenant is using all the capacity you probably only want to fail their requests. Note some designs try and introduce this fairness at a server level, this is generally a bad idea since a tenant can use a lot of capacity globally whilst little per server if it manages to talk to lots of servers at the same time.

For this section we will be assuming elastic modelling of tenants, that is we don't want to reserve capacity for a request that might come in later. The clearest example is if we have one server and some really important requests that must always land on a free machine, under static reservation assumptions we can never serve anything in case an important request is received that needs the machine. All other sections are also based on elastic modelling but rate limiting is an area less common. Servers will often rate limit tenants whilst free-capacity remains. Even with some separate static limit (say 100 requests per tenant), you still need some elastic modelling as well unless the server cannot be overloaded by all tenants firing at their max rate limit which is uncommon. It is very reasonable in some scenarios to rate-limit tenants before all servers are overloaded, and this also makes rate-limiting more predictable rather than being dependent on the overall state of the service. Static rate limit is well understood and out-of-scope here.
